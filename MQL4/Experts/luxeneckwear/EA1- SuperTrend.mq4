...
void order()
  {
   ...
   lotSize = autoLot();
   if(hasValue(superTrendDown_2) && hasValue(superTrendUp_1))/* && superTrendUp_1 < Close[1])*/
     {
      orderBuy();
      orderCloseSell("Order Sell has been closed!");
     }
   else
      if(hasValue(superTrendUp_2) && hasValue(superTrendDown_1))/* && superTrendDown_1 > Close[1])*/
        {
         orderSell();
