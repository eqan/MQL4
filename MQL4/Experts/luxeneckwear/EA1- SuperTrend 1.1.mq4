...
#property description "Trade Entry when the SuperTrend Line changes Color"
#property description "Trade Exit when the SuperTrend Line on higher timeframe changes Color"
#property strict
enum settings
   {
   settings = 0,                                  //======= Settings =======
...
void order()
   ...
   if(hasValue(superTrendDown(Period(),2)) && hasValue(superTrendUp(Period(),1)))
      {
      orderBuy();
      }
   else if(hasValue(superTrendUp(Period(),2)) && hasValue(superTrendDown(Period(),1)))
      {
      orderSell();
      }
   }
